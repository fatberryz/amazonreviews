...
def upload_cleaned_data(service_account_credential_path, project_name, target_dataset, table_name, csv_file_path):
	"""
	This function uploads a csv file into a GBQ table with schemas already defined as a dataframe where the 
	data types can be adjusted to fit the GBQ table schema. 
	Hence, no need for specifying of schema or auto-detecting from CSV
	...
	Parameters:
		project_name: GBQ project name (can be seen from console)
		target_dataset: name of dataset in GBQ
		table_name: determines which table to upload the data into (have to create it in GBQ first with the corresponding schema)
		csv_file_path: path of csv to be uploaded
	"""
	table_id = f"{target_dataset}.{table_name}"
	## Define parameters for load job
	job_config = bigquery.LoadJobConfig(
		#append, empty (only write if doesn't exist) or truncate (overwrite)
		write_disposition=bigquery.WriteDisposition.WRITE_APPEND
	...
	# read csv file to be uploaded into GBQ
	df = pd.read_csv(csv_file_path)

	# change data types to suit GBQ table schema if needed 
	df['date_scraped'] = pd.to_datetime(df['date_scraped'])
	df['review_images'] = df['review_images'].astype(int)

	job = client.load_table_from_dataframe(df, table_id, job_config=job_config)
	# Wait for the load job to complete
	job.result()
	print(f"Loaded {job.output_rows} rows into {table_id} table")
